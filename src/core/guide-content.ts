...
export const PAGE_HINTS: Record<TabId, PageHint> = {
  ...
  stats: {
    ...
    title: '统计',
    brief: '行动选项的生成/选择计数与排行榜，为未来的智能条目管理提供数据基础。',
    points: [
      '全局累计一份统计，只计行动选项视图：AI 每轮成功生成并展示的选项计生成，用户点击应用计选择；润色（enrich）完全不计入。',
      '条目榜按分组折叠展示：命中轮次 = 该条目参与的轮次中、有选项被选中的轮次数（当轮有选择即计 1，整轮共现）；命中率 = 命中轮次 ÷ 参与轮次。有数据的分组默认展开，未参与的折叠显示，可全部展开/收起。',
      '类型榜由条目统计按类型标签聚合推导，口径同上。',
      '「清空统计」会清空全部计数与排行榜，不可撤销。',
    ],
  },
  appearance: {
    icon: 'fa-solid fa-palette',
