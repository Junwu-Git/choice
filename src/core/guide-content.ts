...
export const PAGE_HINTS: Record<string, PageHint> = {
  ...
  status_settings: {
    ...
    points: [
      ...
      '自动更新：开启后每次 AI 回复自动调用 API 提取结构化状态；关闭后仅保留手动编辑。',
      '注入正文：将当前状态（体感条目 + 唤起状态机）以结构化文本注入正文生成，让 AI 在写作时参考。',
      '注入深度：控制提示词插入聊天历史的深度（0=紧接末尾）。',
      '状态条数上限：entries 条数上限，唤起状态机不计入条数。',
    ],
  },
  status_prompt: {
    icon: 'fa-solid fa-align-left',
    ...
    points: [
      '「状态规则」是只读系统模块：定义五类体感 + 唤起状态机、推断边界、结构化 JSON 输出格式。',
      '「状态指令」是用户任务模块：传入上一轮快照让 AI 全量更新，输出 JSON 对象（time_hint / entries / arousal）。',
