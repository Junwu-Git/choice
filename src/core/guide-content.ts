...
export const PAGE_HINTS: Record<TabId, PageHint> = {
  ...
  worldinfo: {
    ...
    points: [
      ...
      '记忆/数据库类插件（如 SP·数据库）写入其注入目标世界书的条目会自动参与选项生成：蓝灯索引/概要条目始终注入，按行拆出的绿灯记忆条目靠关键词命中激活，激活扫描与酒馆主生成同一管线、同一扫描深度。目标书在全局选中 / 角色绑定 / 聊天绑定任一来源即可；不想让某本书参与，用排除列表或三态「关」（含聊天绑定的书）。',
