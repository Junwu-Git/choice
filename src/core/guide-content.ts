    ...
    steps: [
      {
        ...
        title: '外观：面板与悬浮球',
        html: `<p><strong>「悬浮窗」开关</strong>控制屏幕右下角的悬浮球——单击呼出选项弹窗（无需滚动到聊天底部），<strong>右键 / 长按</strong>可查看选项、打开设置或隐藏悬浮球。也可把单击改为直接出设置。</p>
<p><strong>聊天界面</strong>分区的「选项面板」控制聊天流内的选项面板：关闭后改用悬浮球弹窗查看选项，输入润色同步隐藏；停靠位置可选<strong>聊天内</strong>（跟随最新楼层下方）或<strong>输入框上方</strong>（固定不滚、选项再多不占满屏）。</p>`,
        tab: 'appearance',
...
export const PAGE_HINTS: Record<TabId, PageHint> = {
  ...
  stats: {
    ...
    title: '统计',
    brief: '行动选项的生成/选择计数、趋势与排行榜，支持按条目池配置分维度查看与一键应用优选建议。',
    points: [
      '顶部可切换统计维度：「全局」（所有配置混合累计）或某个条目池配置（仅该配置生效会话计入）；未绑定任何配置的会话单独记入「未绑定配置」档。',
      '只计行动选项视图：AI 每轮成功生成并展示的选项计生成，用户点击应用计选择；润色（enrich）完全不计入。',
      '「趋势」按天展示生成/选择活动（7/30 天可切换，随选中维度），自 v51 起累积，历史不回填；卡片提供活跃天数、池内参与率与最近统计时间。',
      '条目榜命中率与「期望」对比：期望 = 按每轮选项数推算的随机基准（4 条时 25%、10 条时 10%），高于期望越多越值得提权，低于越多越值得降权；单个配置维度还显示近 10 轮窗口命中率（50 轮滚动窗口）。',
      '洞察标签仅提示不改权重，但可一键应用：样本充足（≥10 轮）且明显低于期望 →「候选降权/建议停用」，明显高于期望 →「表现良好」；点标签旁的对勾或「应用全部建议」把权重修改写入当前配置（可撤销）。',
      '「样本分布」按当前维度有效池展示样本充足/不足/从未参与三档，充足占比越高，优化建议越可信。',
      '「导出 JSON」下载完整统计（各维度总量、逐日计数、条目统计与条目库 join 信息）；「清空统计」清空全部维度，不可撤销。',
    ],
  },
  appearance: {
    icon: 'fa-solid fa-palette',
    title: '外观',
    brief: '悬浮窗、聊天界面面板、主题与字体大小。',
    points: [
